...
binary:
	cd cmd/dcgm-exporter; GOEXPERIMENT=boringcrypto $(GO) build -trimpath -tags fips -ldflags "-X main.BuildVersion=${DCGM_VERSION}-${VERSION}" && go tool nm $(PREFIX_DIR)/$* | grep -E 'sig.FIPSOnly'
